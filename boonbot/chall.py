...
class JoinButton(View):
    ...
    async def join_button(self, interaction: discord.Interaction, button: Button):
        ...
        await interaction.response.send_message(
            f"スレッドに参加しました {CHECK_EMOJI}", ephemeral=True
        )
...
class AutoJoinButton(View):
    ...
    @discord.ui.button(
        label="問題スレッドに自動で参加", style=discord.ButtonStyle.success
    )
...
async def new_chall(ctx: discord.Interaction, category: Categories, problem_name: str):
    ...
    adding_users = set(
        ...
        for user_id in auto_join_users.get(ctx.channel.id) or []
    )
...
async def new_ctf(
    ctx: discord.Interaction, ctf_name: str, role_name: str, need_reaction: bool = False
):
    if not await util.check_is_in_bot_cmd(ctx):
        ...
        resp_text = (
            f"{role_name}に{ctf_name} ({channel.mention}) を作成しました {CHECK_EMOJI}"
        )
        if need_reaction:
...
async def end_ctf(ctx: discord.Interaction):
    ...
    for ch in [_ch for _ch in ctx.guild.channels if _ch.name == ctx.channel.name]:
        ...
        whitelisted_members = list(
            filter(lambda u: ch.overwrites_for(u).read_messages, non_bot_member)
        )
